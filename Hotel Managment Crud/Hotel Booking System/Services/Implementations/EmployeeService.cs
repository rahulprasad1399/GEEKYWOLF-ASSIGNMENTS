...
namespace Hotel_Booking_System.Services.Implementations
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<Employee> CreateEmployee(Employee employee)
        {
            ...
            bool emailExist = await _context.employees.AnyAsync(x => x.Email == employee.Email);
            if (emailExist)
            {
                throw new Exception("Email already exist");
            }

            bool hotelExist = await _context.hotels.AnyAsync(x => x.Id == employee.HotelId);
            if (!hotelExist)
            {
                throw new Exception($"Hotel Id {employee.HotelId} dosent exist");
            }
